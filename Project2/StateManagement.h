...
public
	...
	private:
		System::Windows::Forms::ImageList ^ imageList5;

	private:
		System::Windows::Forms::ImageList ^ imageList6;

	private:
		System::Windows::Forms::ImageList ^ imageList7;
	...
	public:
		...
		int getImgIndex(std::string foodName)
		{
			...
			else if (index == 2)
			{
				...
				else if (foodName == "Tomato Basil Soup")
				{
					...
					return 4;
				else if (foodName == "White Bean Chicken Soup")
					return 5;
				else
					return -1;
			}
			else
			{
				...
				else if (foodName == "Bun Dau Doi Sun")
					return 3;
				else if (foodName == "Bun Dau Thap Cam")
					return 5;
				else
					return -1;
			}
		...
		void resetCart(int prevIndex)
		{
			...
			if (prevIndex == 1)
				...
				n = CartData::getNumberOfBundaumamtom();
			for (int i = 0; i < n; i++)
			{
				layouts[i]->Hide();
			}
		...
		void updateCart()
		{
			...
			else if (index == 2)
			{
				...
				data = CartData::getBundaumamtomData();
			...
			for (int i = 0; i < n; i++)
			{
				...
				else if (index == 2)
				{
					...
					icons[i]->BackgroundImage = imageList7->Images[imgIndex];
	...
	private:
		System::Void exitBtn_Click(System::Object ^ sender, System::EventArgs ^ e)
		{
			...
			else if (index == 2)
			{
				...
				CartData::removeBundaumamtom(stoi(id));
